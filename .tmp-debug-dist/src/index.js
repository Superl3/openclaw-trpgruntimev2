...
import { assertAgentAllowed, parseTrpgRuntimeConfig, resolveWorldRootForContext, trpgRuntimeConfigSchema, } from "./config.js";
import { runDiceRoll } from "./dice.js";
import { createPatchCache, runPatchApply, runPatchDryRun, } from "./patch-engine.js";
import { formatFactionPromptSummary, runFactionEngineTick, } from "./faction-engine.js";
import { loadStructuredWorldFile, renderStructuredContent, resolveWorldAbsolutePath, runHooksQuery, runStoreGet, } from "./world-store.js";
import { buildLifecycleCompactionPreview, runStateCompactionTool, } from "./lifecycle-compact.js";
import { buildSceneComponents, GAME_COMPONENT_USAGE_GUIDE, NON_GAME_COMPONENT_USAGE_GUIDE, } from "./discord-components.js";
...
function readSceneId(scene) {
    return readString(scene.scene_id) || readString(scene.id) || "unknown-scene";
}
function parseSceneIdFromTick(lastAdvancedTick) {
    const marker = ":scene-";
...
function buildPromptSurfaceChunk(params) {
    ...
    const profileByTag = {
        ...
        TRPG_DISCORD_COMPONENTS_BOOTSTRAP: { mandatory: true, priority: 83, maxLines: 12, maxChars: 1200 },
...
function applyPromptInjectionBudget(params) {
    ...
    const surfaces = params.chunks
        .map((chunk, index) => buildPromptSurfaceChunk({
        text: chunk,
        order: index,
...
function daysSinceIso(value, nowMs) {
...
function zoneTypeSignificance(type) {
...
function toLifecycleState(value) {
...
function coerceFiniteInteger(value, fallback) {
...
function parseCompactionTrigger(value) {
...
function parseCompactionMode(value) {
    const normalized = readString(value);
...
function hasExplicitLifecycleToolInvocationIntent(latestAction) {
    const lower = latestAction.trim().toLowerCase();
    if (!lower) {
        return false;
    }
    return (lower.includes("trpg_state_compact") ||
        lower.includes("state_compact") ||
        lower.includes("state compact") ||
        lower.includes("lifecycle compaction"));
}
function isDowntimeTickIntent(latestAction) {
    const lower = latestAction.trim().toLowerCase();
    if (!lower) {
        return false;
    }
    return (lower.includes("downtime_tick") ||
...
function mapFallbackTriggerToCompactionTrigger(trigger) {
...
async function runLifecyclePreviewIfNeeded(params) {
    ...
    if (hasExplicitLifecycleToolInvocationIntent(params.latestAction)) {
        params.api.logger.info("[trpg-runtime] lifecycle fallback skipped trigger=" +
            params.trigger +
    ...
    try {
        ...
        params.api.logger.info("[trpg-runtime] lifecycle fallback dry-run trigger=" +
            params.trigger +
            " compaction_trigger=" +
            compactionTrigger +
            " candidates=" +
        ...
        params.api.logger.warn("[trpg-runtime] lifecycle fallback preview skipped: " +
...
function countSeedThreadsByZone(worldSeedsRoot) {
    const result = {};
...
function collectFactionLinkedZones(factionRoot) {
    const out = new Set();
...
function makeDefaultCompactionStateRoot() {
    return {
        meta: {
            schema_version: 1,
...
function makeDefaultArchiveSummaryRoot() {
    return {
        meta: {
            schema_version: 1,
            last_updated: new Date().toISOString(),
...
function extractLatestUserMessageFromPrompt(prompt) {
    if (!prompt) {
        return "";
    }
    ...
    for (let index = lines.length - 1; index >= 0; index -= 1) {
        ...
        const joined = sanitizeBootstrapUserFacingText(chunks.join(String.fromCharCode(10)).trim());
        if (joined) {
            return joined;
        }
    }
    ...
    if (fallbackLines.length > 0) {
        return sanitizeBootstrapUserFacingText(fallbackLines.reverse().join(String.fromCharCode(10)).trim());
    }
    return "";
}
function parseLabeledAnswer(message, labels) {
    const escapedLabels = labels.map((label) => label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
    const pattern = new RegExp("(?:^|\\n)\\s*(?:" + escapedLabels.join("|") + ")\\s*[:：\\-]\\s*(.+)", "i");
    const matched = message.match(pattern);
    return matched && matched[1] ? matched[1].trim() : "";
}
function parseNumberedAnswers(message) {
    const lines = message
        .split(String.fromCharCode(10))
        .map((line) => line.trim())
        .filter(Boolean);
...
function parseBootstrapUpdate(message) {
    const update = {
    ...
    for (const [key, labels] of Object.entries(labeledCandidates)) {
        if (update[key]) {
            continue;
        }
...
function hasBootstrapReadySignal(message) {
    if (!message) {
        return false;
    }
    return /(준비(?:됐|되었습니다|완료|끝)|시작(?:해|하자|하겠습니다)|진행해|이제\s*가자|ready|let'?s\s*go|go\s*ahead)/i.test(message);
}
function collectMissingBootstrapFields(player) {
    const missing = [];
    if (!readString(player.name))
        missing.push("이름");
    if (!readString(player.background))
        missing.push("출신 / 배경");
    if (!readString(player.motive))
        missing.push("지금 이 세계에 들어온 이유");
    if (!readString(player.fear))
        missing.push("두려워하는 것");
    if (!readString(player.goal))
        missing.push("지금 당장의 목표");
    return missing;
}
const BOOTSTRAP_SAFE_DESCRIPTION_FALLBACK = "캐릭터 준비를 이어갈게요. 필요한 정보는 한 단계씩 선택하거나 자유롭게 알려주세요.";
function stripCodeFenceBlocks(value) {
    return value.replace(/```[\s\S]*?```/g, " ");
}
function isLikelyPromptWrapperLine(line) {
    if (!line) {
        return true;
    }
    if (/^<{1,2}[!?/]?[a-z][\w:-]*(?:\s+[^>]*)?>/i.test(line) || /^<\/[a-z][\w:-]*>/i.test(line)) {
        return true;
    }
    if (/^(?:current\s*time|untrusted\s*context|execution[_\s-]*packet|execution[_\s-]*policy|manager[_\s-]*route|acceptance[_\s-]*lock|policy|contract|payload|schema[_\s-]*version|doc[_\s-]*id|run[_\s-]*id|task[_\s-]*id|must[_\s-]*do|must[_\s-]*not[_\s-]*do)\b/i.test(line)) {
        return true;
    }
    if (/^(?:you are powered by|the exact model id|working directory|workspace root folder|is directory a git repo|platform|today'?s date)\b/i.test(line)) {
        return true;
    }
    if (/^\s*["']?(?:doc_id|run_id|task_id|schema_version|execution_packet|acceptance_lock|content_sha256)["']?\s*:/i.test(line)) {
        return true;
    }
    if (/^(?:<pxml|<meta>|<refs>|<payload>|<integrity>|<item>|<ref>|<doc_id>|<doc_class>|<sequence>|<writer_agent>|<created_at>)/i.test(line)) {
        return true;
    }
    return false;
}
function containsForbiddenBootstrapTemplate(value) {
    if (!value) {
        return false;
    }
    const normalized = value.toLowerCase();
    if (/\bpart\s*a\b/.test(normalized) ||
        /\bpart\s*b\b/.test(normalized) ||
        /좋아요\s*,?\s*새\s*캐릭터\s*생성을\s*시작할게요/i.test(value) ||
        /숨기고\s*있는\s*비밀/i.test(value)) {
        return true;
    }
    const numberedMatches = value.match(/(?:^|\n)\s*(?:[1-6])\s*[\).:：-]\s+/g);
    return Array.isArray(numberedMatches) && numberedMatches.length >= 4;
}
function sanitizeBootstrapDescriptionText(value) {
    const sanitized = sanitizeBootstrapUserFacingText(value);
    if (!sanitized) {
        return BOOTSTRAP_SAFE_DESCRIPTION_FALLBACK;
    }
    if (containsForbiddenBootstrapTemplate(sanitized)) {
        return BOOTSTRAP_SAFE_DESCRIPTION_FALLBACK;
    }
    return sanitized;
}
function sanitizeBootstrapFreeformStorageText(value) {
    const sanitized = sanitizeBootstrapUserFacingText(value);
    if (!sanitized || containsForbiddenBootstrapTemplate(sanitized)) {
        return "";
    }
    return sanitized;
}
function sanitizeBootstrapUserFacingText(value) {
    if (!value) {
        return "";
    }
    const lines = stripCodeFenceBlocks(value)
        .split(String.fromCharCode(10))
        .map((line) => line.replaceAll(String.fromCharCode(13), "").trim())
        .filter(Boolean)
        .filter((line) => !isLikelyPromptWrapperLine(line));
    if (lines.length === 0) {
        return "";
    }
    const deduped = Array.from(new Set(lines));
    return deduped.join(String.fromCharCode(10)).trim();
}
function deriveSystemSecret(player) {
    const name = readString(player.name) || "traveler";
    const motives = [
        "과거의 맹세를 어겼다는 사실을 숨기고 있다",
        "이 지역의 오래된 사건과 직접 연루되어 있다",
        "누군가의 신분을 빌려 들어왔다는 흔적이 남아 있다",
        "특정 조직과의 은밀한 거래 기록을 감추고 있다",
    ];
    const index = deterministicIndex(`${name}:${readString(player.background)}:${readString(player.motive)}`, motives.length);
    return motives[index] || motives[0];
}
function isCanonicalWriteBackDisabledMessage(value) {
    return /canonical write-back is disabled/i.test(value);
}
function hasMinimalBootstrapFields(player) {
    const keys = ["name", "background", "motive", "secret", "fear", "goal"];
    let answered = 0;
...
function extractBootstrapFreeform(message) {
    if (!message) {
        return "";
    }
    const lines = stripCodeFenceBlocks(message)
        .split(String.fromCharCode(10))
        .map((line) => line.trim())
        .filter(Boolean);
    const numberedPattern = /^(1|2|3|4|5|6)(?:\s*번)?[\).:：\-\.\s]+/;
    const labeledPattern = /^(이름|출신|배경|출신\s*\/\s*배경|지금\s*이\s*세계에\s*들어온\s*이유|숨기고\s*있는\s*비밀|두려워하는\s*것|지금\s*당장의\s*목표|name|origin|motive|secret|fear|goal)\s*[:：\-]/i;
    const freeformLines = lines.filter((line) => {
        ...
        if (isLikelyPromptWrapperLine(line)) {
            return false;
        }
        if (line.startsWith("/")) {
            return false;
    ...
    return sanitizeBootstrapUserFacingText(freeformLines.join(String.fromCharCode(10)).trim());
}
function mergeFreeformDescription(existingValue, incomingValue) {
    const existing = sanitizeBootstrapFreeformStorageText(existingValue.trim());
    const incoming = sanitizeBootstrapFreeformStorageText(incomingValue.trim());
    if (!incoming) {
        return existing;
    }
    if (!existing) {
...
function isMovementIntent(message) {
    ...
    const travelVerbPattern = /(이동|간다|가겠다|향한다|향해|출발|따라|샌다|새다|빠진다|우회|내려간다|go|move|head|travel|sail|ride)/;
    const stationaryPattern = /(이동하지\s*않|움직이지\s*않|한\s*발도\s*움직이지|제자리|가만히|멈춘\s*채|멈춰\s*서|그대로|주변만\s*살핀)/;
    if (stationaryPattern.test(lower) && !travelVerbPattern.test(lower)) {
        return false;
    }
    return travelVerbPattern.test(lower);
}
function buildZoneGraph(parsedPressure) {
    const pressureRoot = toObject(parsedPressure);
    const graph = {};
    const addNode = (rawNode, fallbackId = "") => {
        ...
        const parsedPressure = readFiniteNumber(rawNode.pressure) ?? readFiniteNumber(rawNode.score) ?? existing?.pressure ?? null;
        graph[id] = {
            id,
            ...
            parentRegion: readString(rawNode.parent_region) ||
    ...
    for (const [zoneId, nearbyValue] of Object.entries(topology)) {
        ...
        graph[normalizedZoneId].connections = Array.from(new Set([...graph[normalizedZoneId].connections, ...nearbyIds]));
    }
...
function buildZoneAliasMap(zoneGraph) {
    const out = {};
...
function extractZoneMentions(params) {
    ...
    return Object.values(mentions).sort((a, b) => a.firstIndex === b.firstIndex ? a.lastIndex - b.lastIndex : a.firstIndex - b.firstIndex);
}
function resolveTravelIntent(params) {
    ...
    if (mentionedZoneIds.length >= 2) {
        pathZoneId = mentionedZoneIds[0];
        destinationZoneId = mentionedZoneIds[mentionedZoneIds.length - 1];
    }
    else if (mentionedZoneIds.length === 1) {
        destinationZoneId = mentionedZoneIds[0];
    }
...
function shortestPath(zoneGraph, start, goal) {
...
function zoneName(zoneGraph, zoneId) {
    return zoneGraph[zoneId]?.name || zoneId;
}
function zoneType(zoneGraph, zoneId) {
    return zoneGraph[zoneId]?.type || "unknown";
}
function pressureSignalsForZone(pressureParsed, zoneId) {
...
function travelHintsByZoneType(value) {
...
function inferZoneTypeFromLabel(label) {
    const lower = label.toLowerCase();
    if (/(항구|선창|부두|항로|harbor|dock|port|sea)/i.test(lower))
        return "port";
    if (/(숲|산길|습지|forest|wild|wilderness)/i.test(lower))
        return "wilderness";
    if (/(폐허|유적|사원|성소|ruin|shrine|dungeon|catacomb)/i.test(lower))
        return "ruin";
    if (/(가도|도로|길|road|frontier|checkpoint)/i.test(lower))
        return "road";
    return "settlement";
}
function normalizeUnknownDestinationLabel(raw) {
    let candidate = raw
        .replace(/^["'“”‘’]+|["'“”‘’]+$/g, "")
        .replace(/\s+/g, " ")
        .trim();
    ...
    candidate = candidate
        .replace(/\s*(?:으)?로\s*(?:이동(?:한다|해|하겠다|할게)?|간다|가겠다|가자|향한다|향해|출발(?:한다|해)?|move|go|travel|head(?:s|ing)?).*$/i, "")
        .replace(/\s*에\s*(?:이동(?:한다|해|하겠다|할게)?|간다|가겠다|가자|향한다|향해|출발(?:한다|해)?|도착(?:한다|해)?|move|go|travel|head(?:s|ing)?).*$/i, "")
        .replace(/[.,!?;:]+$/g, "")
        .trim();
...
function isKnownDestinationAlias(candidate, aliasMap) {
...
function extractUnknownDestinationLabel(message, aliasMap) {
...
async function generateLinkedZoneForUnknownDestination(params) {
    ...
    if (!zoneId || params.zoneGraph[zoneId])
        return null;
    ...
    for (const linked of links)
        topo[linked] = uniqStrings([...toStringArray(topo[linked]), zoneId]);
    ...
    const rendered = renderStructuredContent(params.pressureFormat, pressureRoot);
...
async function appendZoneSeeds(params) {
    ...
    const types = ["rumor", "hidden_location", "npc_connection", "faction_interest", "environmental_mystery"];
    const hints = [];
    let changed = false;
...
async function runTravelMovement(params) {
    const latestUserMessage = extractLatestUserMessageFromPrompt(params.prompt) || extractLatestUserMessage(params.messages);
    ...
    const currentZoneId = normalizeZoneId(readString(travelState.current_zone) || sceneZone || zoneIds[0] || "");
    ...
    const shouldPreferGeneratedDestination = Boolean(explicitUnknownDestination && !isKnownDestinationAlias(explicitUnknownDestination, aliasMap));
    ...
    let immediateTargetZoneId = travelIntent.pathZoneId && travelIntent.pathZoneId !== currentZoneId
...
async function detectSceneTransition(params) {
    ...
    const persistedSceneId = readString(engineState.last_scene_id) ||
...
export function resolveRuntimePhase(params) {
    const playerRoot = toObject(params.playerCanonParsed);
    ...
    const canonCharacterCreated = readBoolean(gameState.character_created);
    const canonBootstrapComplete = readBoolean(gameState.bootstrap_complete);
    const scenePlayerSetupComplete = readBoolean(scene.player_setup_complete) || readBoolean(sceneFlow.player_setup_complete);
    const sceneIntroShown = readBoolean(sceneFlow.intro_shown);
    const characterCreated = canonCharacterCreated || Boolean(readString(player.name));
    // intro_shown can be stale from prior runs. Phase authority must come from
    // canonical bootstrap completion and setup-complete flags only.
    const bootstrapComplete = canonBootstrapComplete || scenePlayerSetupComplete;
    let phase = "IN_GAME";
    if (!characterCreated) {
        phase = "BOOTSTRAP";
    }
    ...
    return {
        ...
        bootstrapComplete,
        evidence: {
            canonCharacterCreated,
            canonBootstrapComplete,
            scenePlayerSetupComplete,
            sceneIntroShown,
        },
    };
}
const NON_GAME_ALLOWED_BUTTON_LABELS = new Set([
    "🆕 새 캐릭터 시작",
    "📂 캐릭터 불러오기",
    "🧷 초기 정보 유지",
    "❌ 닫기",
]);
const NON_GAME_DEFAULT_BUTTONS = [
    { label: "🆕 새 캐릭터 시작", style: "primary" },
    { label: "📂 캐릭터 불러오기", style: "secondary" },
    { label: "🧷 초기 정보 유지", style: "success" },
];
function toButtonStyle(value) {
    return value === "primary" || value === "secondary" || value === "success" || value === "danger"
        ? value
        : "secondary";
}
function normalizeSceneInputForNonGame(raw) {
    const filteredButtons = Array.isArray(raw.buttons)
        ? raw.buttons
            .map((entry) => ({
            label: readString(entry?.label),
            style: toButtonStyle(entry?.style),
        }))
            .filter((entry) => NON_GAME_ALLOWED_BUTTON_LABELS.has(entry.label))
        : [];
    return {
        ...raw,
        scene: raw.scene === "choice" && Array.isArray(raw.choices) && raw.choices.length > 0 ? "choice" : "system",
        description: sanitizeBootstrapDescriptionText(readString(raw.description)),
        locationInfo: "",
        npc: undefined,
        combat: undefined,
        inventoryChoices: [],
        includeInput: false,
        buttons: filteredButtons.length > 0 ? filteredButtons : NON_GAME_DEFAULT_BUTTONS,
    };
}
const NON_GAME_SELECT_MAX_OPTIONS = 8;
const NON_GAME_SELECT_MAX_LABEL_LENGTH = 60;
const NON_GAME_SELECT_MAX_VALUE_LENGTH = 80;
const NON_GAME_SELECT_MAX_DESCRIPTION_LENGTH = 100;
const NON_GAME_SELECT_CUSTOM_ID_PATTERN = /^[a-zA-Z0-9._:-]{3,80}$/;
function normalizeNonGameSelect(selectRaw) {
    const select = toObject(selectRaw);
    const optionsRaw = Array.isArray(select.options) ? select.options : [];
    if (optionsRaw.length === 0 || optionsRaw.length > NON_GAME_SELECT_MAX_OPTIONS) {
        return null;
    }
    const customId = readString(select.custom_id || select.customId);
    if (customId && !NON_GAME_SELECT_CUSTOM_ID_PATTERN.test(customId)) {
        return null;
    }
    const options = [];
    for (const optionRaw of optionsRaw) {
        const option = toObject(optionRaw);
        ...
        if (label.length > NON_GAME_SELECT_MAX_LABEL_LENGTH || value.length > NON_GAME_SELECT_MAX_VALUE_LENGTH) {
            return null;
        }
        if (description.length > NON_GAME_SELECT_MAX_DESCRIPTION_LENGTH) {
            return null;
        }
    ...
    return {
        type: "string",
        placeholder: readString(select.placeholder).slice(0, 80) || "선택하세요...",
        ...(customId ? { custom_id: customId } : {}),
        options,
    };
}
function normalizeBuiltComponentsForNonGame(components) {
    const normalized = toObject(components);
    const embeds = Array.isArray(normalized.embeds) ? normalized.embeds.map((entry) => toObject(entry)) : [];
    if (embeds.length > 0) {
        const first = embeds[0];
        first.title = "⚙️ 시스템 안내";
        first.description = sanitizeBootstrapDescriptionText(readString(first.description));
        embeds[0] = first;
        normalized.embeds = embeds;
    }
    const blocksRaw = Array.isArray(normalized.blocks) ? normalized.blocks.map((entry) => toObject(entry)) : [];
    const blocks = [];
    for (const block of blocksRaw) {
        if (block.select) {
            const select = normalizeNonGameSelect(block.select);
            if (select) {
                blocks.push({ type: "actions", select });
            }
            continue;
        }
        const buttonsRaw = Array.isArray(block.buttons) ? block.buttons : null;
        if (!buttonsRaw) {
            if (block.type === "text") {
                blocks.push({
                    ...block,
                    text: sanitizeBootstrapDescriptionText(readString(block.text)),
                });
                continue;
            }
            blocks.push(block);
        ...
        const filteredButtons = buttonsRaw
            .map((entry) => {
            const button = toObject(entry);
            const label = readString(button.label);
            if (!NON_GAME_ALLOWED_BUTTON_LABELS.has(label)) {
                return null;
            }
            return {
                label,
                style: toButtonStyle(button.style),
            };
        })
    ...
    if (!blocks.some((entry) => {
        const selectOptions = toObject(entry.select).options;
        return ((Array.isArray(entry.buttons) && entry.buttons.length > 0) ||
            (Array.isArray(selectOptions) && selectOptions.length > 0));
    })) {
        blocks.push({
            type: "actions",
            buttons: NON_GAME_DEFAULT_BUTTONS,
        });
    }
...
function visibilityHint(value) {
    ...
    if (!normalized)
        return "unknown";
    if (["public", "introduced", "revealed", "known", "open"].includes(normalized))
        return "show";
    if (["hidden", "secret", "unrevealed", "private", "masked"].includes(normalized))
        return "hide";
    return "unknown";
}
function isNpcNameVisibleToPlayer(npc) {
    ...
    for (const key of keys)
        if (readBooleanFlag(npc[key]) === true)
            return true;
    for (const key of ["name_visibility", "identity_visibility", "disclosure_state", "reveal_state"]) {
    ...
    for (const key of keys)
        if (readBooleanFlag(npc[key]) === false)
            return false;
    return true;
}
function npcMaskLabel(npc, index) {
    const role = readString(npc.role);
    if (role)
        return `${role} (name withheld)`;
    const id = readString(npc.id);
    if (id)
        return `${id} (name withheld)`;
    return `unidentified-npc-${index + 1}`;
}
function collectSceneNpcVisibility(parsed) {
...
function redactHiddenNpcNames(value, npcVisibility) {
    if (!value)
        return "";
...
function buildNpcVisibilityGuardChunk(parsed) {
    ...
    for (const entry of hiddenNpcs.slice(0, 8))
...
function formatGauge(label, current, max) {
...
function sanitizeIntentText(value, maxLength = 240) {
    if (!value) {
        return "";
    }
...
function deterministicIndex(seed, modulo) {
    ...
    const digest = createHash("sha256").update(seed || "seed", "utf8").digest("hex");
    const parsed = Number.parseInt(digest.slice(0, 8), 16);
    if (!Number.isFinite(parsed)) {
        return 0;
    }
    return Math.abs(parsed) % modulo;
}
function pickDeterministicSubset(seed, source, minCount, maxCount) {
    if (source.length === 0) {
        return [];
    }
    const minSafe = Math.max(0, minCount);
    const maxSafe = Math.max(minSafe, Math.min(maxCount, source.length));
    const count = minSafe + deterministicIndex(seed + ":count", maxSafe - minSafe + 1);
    const ranked = source
        .map((entry, index) => ({
        entry,
        rank: deterministicSample(`${seed}:${entry}:${String(index)}`),
    }))
        .sort((a, b) => a.rank - b.rank)
        .map((entry) => entry.entry);
    return ranked.slice(0, Math.max(0, count));
}
function makeDefaultPlayerStatusRoot(seedBase) {
    const hpMax = 12;
    const staminaMax = 10;
    const stressMax = 10;
    const money = 30;
    return {
        ...
        player_status: {
            ...
            stamina_gauge: { current: staminaMax, max: staminaMax },
        ...
        status: {
            health: { current: hpMax, max: hpMax },
            stamina: { current: staminaMax, max: staminaMax },
            stress: { current: 0, max: stressMax },
            economy: { money, funds: money, currency: "coins" },
        },
    };
}
function makeDefaultInventoryRoot(seedBase) {
    const carriedPool = ["water flask", "travel cloak", "torch", "bandage kit", "old map", "ration pouch"];
    const equippedPool = ["rusted dagger", "walking staff", "worn buckler"];
    const notesPool = ["start: basic provisions packed", "start: local rumors pending", "start: unknown zone marker"];
    const carried = pickDeterministicSubset(`${seedBase}:carried`, carriedPool, 1, 3);
    const equipped = pickDeterministicSubset(`${seedBase}:equipped`, equippedPool, 0, 1);
    const notes = pickDeterministicSubset(`${seedBase}:notes`, notesPool, 1, 2);
    return {
        meta: {
            schema_version: 1,
            last_updated: new Date().toISOString(),
...
async function loadStructuredWorldFileLenient(params) {
    try {
        const loaded = await loadStructuredWorldFile(params.worldRoot, params.relativePath, {
            allowMissing: true,
            maxReadBytes: params.maxReadBytes,
        });
        return { loaded, parseCorrupt: false };
    }
    catch {
        return {
            loaded: {
                format: params.relativePath.endsWith(".json") ? "json" : "yaml",
                parsed: {},
                exists: false,
            },
            parseCorrupt: true,
...
async function ensureStatusAndInventorySeed(params) {
    const [statusLoaded, inventoryLoaded] = await Promise.all([
        loadStructuredWorldFileLenient({
            worldRoot: params.worldRoot,
            relativePath: "state/player-status.yaml",
            maxReadBytes: params.cfg.maxReadBytes,
        }),
        loadStructuredWorldFileLenient({
            worldRoot: params.worldRoot,
            relativePath: "state/inventory.yaml",
            maxReadBytes: params.cfg.maxReadBytes,
        }),
    ]);
    const statusRoot = toObject(statusLoaded.loaded.parsed);
    const statusNode = toObject(statusRoot.status);
    const health = toObject(statusNode.health);
    const staminaGauge = toObject(statusNode.stamina);
    const stress = toObject(statusNode.stress);
    const economy = toObject(statusNode.economy);
    const playerStatus = toObject(statusRoot.player_status);
    let statusChanged = statusLoaded.parseCorrupt || !statusLoaded.loaded.exists;
    if (readFiniteNumber(health.max) === null) {
        health.max = 12;
        statusChanged = true;
    }
    if (readFiniteNumber(health.current) === null) {
        health.current = readFiniteNumber(health.max) ?? 12;
        statusChanged = true;
    }
    if (readFiniteNumber(staminaGauge.max) === null) {
        staminaGauge.max = 10;
        statusChanged = true;
    }
    if (readFiniteNumber(staminaGauge.current) === null) {
        staminaGauge.current = readFiniteNumber(staminaGauge.max) ?? 10;
        statusChanged = true;
    }
    if (readFiniteNumber(stress.max) === null) {
        stress.max = 10;
        statusChanged = true;
    }
    if (readFiniteNumber(stress.current) === null) {
        stress.current = 0;
        statusChanged = true;
    }
    if (readFiniteNumber(playerStatus.money) === null && readFiniteNumber(economy.money) === null && readFiniteNumber(economy.funds) === null) {
        playerStatus.money = 30;
        economy.money = 30;
        economy.funds = 30;
        statusChanged = true;
    }
    if (!readString(playerStatus.stamina_state) && !readString(playerStatus.stamina)) {
        playerStatus.stamina_state = "normal";
        playerStatus.stamina = "normal";
        statusChanged = true;
    }
    else if (!readString(playerStatus.stamina_state) && readString(playerStatus.stamina)) {
        playerStatus.stamina_state = readString(playerStatus.stamina);
        statusChanged = true;
    }
    ...
    statusNode.health = health;
    statusNode.stamina = staminaGauge;
    statusNode.stress = stress;
    statusNode.economy = economy;
    statusRoot.status = statusNode;
    ...
    const inventoryRoot = toObject(inventoryLoaded.loaded.parsed);
    const inventoryNode = toObject(inventoryRoot.inventory);
    let inventoryChanged = inventoryLoaded.parseCorrupt || !inventoryLoaded.loaded.exists;
    const defaultInventory = makeDefaultInventoryRoot(params.seedBase);
    const defaultNode = toObject(toObject(defaultInventory).inventory);
    const carried = uniqStrings(toStringArray(inventoryNode.carried));
    if (carried.length === 0) {
        inventoryNode.carried = toStringArray(defaultNode.carried);
        inventoryChanged = true;
    }
    const equipped = uniqStrings(toStringArray(inventoryNode.equipped));
    if (equipped.length === 0 && deterministicIndex(`${params.seedBase}:equipped-enable`, 2) === 1) {
        inventoryNode.equipped = toStringArray(defaultNode.equipped);
        inventoryChanged = true;
    }
    else if (!Array.isArray(inventoryNode.equipped)) {
        inventoryNode.equipped = [];
        inventoryChanged = true;
    }
    const notes = toStringArray(inventoryNode.notes);
    if (notes.length === 0) {
        inventoryNode.notes = toStringArray(defaultNode.notes);
        inventoryChanged = true;
    }
    inventoryRoot.inventory = inventoryNode;
    if (inventoryChanged) {
        inventoryRoot.meta = {
    ...
    const writes = [];
    if (statusChanged) {
        const statusAbsolute = resolveWorldAbsolutePath(params.worldRoot, "state/player-status.yaml");
        await fs.mkdir(path.dirname(statusAbsolute), { recursive: true });
        writes.push(fs.writeFile(statusAbsolute, renderStructuredContent(statusLoaded.loaded.format, statusRoot), "utf8"));
    }
    if (inventoryChanged) {
        const inventoryAbsolute = resolveWorldAbsolutePath(params.worldRoot, "state/inventory.yaml");
        await fs.mkdir(path.dirname(inventoryAbsolute), { recursive: true });
        writes.push(fs.writeFile(inventoryAbsolute, renderStructuredContent(inventoryLoaded.loaded.format, inventoryRoot), "utf8"));
    }
    if (writes.length > 0) {
        await Promise.all(writes);
    }
}
function buildInventoryTokenSet(status) {
    return new Set([...status.carriedItems, ...status.equippedItems, ...status.inventoryHighlights]
        .map((entry) => normalizeInventoryToken(entry))
        .filter(Boolean));
}
function hasInventoryTokenMatch(inventoryTokens, token) {
...
function extractLegacyInventory(entries) {
    const carried = [];
    const equipped = [];
    const highlights = [];
    for (const entry of entries) {
        ...
        const display = quantity !== null && quantity > 1
...
async function loadStatusPanelData(params) {
    await ensureStatusAndInventorySeed({
        cfg: params.cfg,
        worldRoot: params.worldRoot,
    ...
    const [statusLoaded, inventoryLoaded, sceneLoaded] = await Promise.all([
        ...
        loadStructuredWorldFileLenient({
            worldRoot: params.worldRoot,
            relativePath: "state/current-scene.yaml",
            maxReadBytes: params.cfg.maxReadBytes,
        }),
    ]);
    const statusRoot = toObject(statusLoaded.loaded.parsed);
    ...
    const sceneRoot = toObject(sceneLoaded.loaded.parsed);
    const panelTimeIso = readString(toObject(statusRoot.meta).last_updated) ||
        readString(toObject(sceneRoot.meta).last_updated) ||
    ...
    const money = readFiniteNumber(playerStatus.money) ??
        readFiniteNumber(economy.money) ??
        readFiniteNumber(economy.funds);
    const fundsText = money !== null
        ? `coins ${String(Math.round(money))}`
        : readString(economy.funds) || readString(economy.currency) || "unknown";
    return {
        panelTimeIso,
        hpCurrent: readFiniteNumber(health.current) ?? readFiniteNumber(toObject(playerStatus.hp).current),
        hpMax: readFiniteNumber(health.max) ?? readFiniteNumber(toObject(playerStatus.hp).max),
        staminaCurrent: readFiniteNumber(staminaGauge.current) ?? readFiniteNumber(toObject(playerStatus.stamina_gauge).current),
        staminaMax: readFiniteNumber(staminaGauge.max) ?? readFiniteNumber(toObject(playerStatus.stamina_gauge).max),
        stressCurrent: readFiniteNumber(stress.current) ?? readFiniteNumber(toObject(playerStatus.stress).current),
        stressMax: readFiniteNumber(stress.max) ?? readFiniteNumber(toObject(playerStatus.stress).max),
        money,
        staminaState: readString(playerStatus.stamina_state) || readString(playerStatus.stamina) || "normal",
...
function isStatusRecallIntent(message) {
    ...
    return /(상태창|상태\s*(확인|보여|요약)|스탯|체력\s*상태|인벤|소지품|장비\s*확인|status\s*(check|panel|recall)|inventory\s*(check|recall))/i.test(message);
}
function buildStatusPanelGuardChunk(params) {
    if (!isInGamePhase(params.runtimePhase)) {
        return "";
    }
    const lines = [
        "[TRPG_RUNTIME_STATUS_PANEL_V1]",
        `World time: ${params.status.panelTimeIso}`,
    ...
    if (params.status.money === null && params.status.fundsText && params.status.fundsText !== "unknown") {
        lines.push(`Money alias(Funds): ${params.status.fundsText}`);
    }
...
function parseEconomyPurchaseIntent(message) {
    if (!message) {
        return null;
    }
    ...
    if (!item) {
        const inferred = message.match(/([가-힣a-zA-Z0-9][가-힣a-zA-Z0-9\s\-]{1,32})(?:을|를)?\s*(?:\d+\s*(?:은화|골드|coin|coins|money|금화|코인)\s*)?(?:구매|구입|산다|buy|purchase|procure)/i);
...
function detectInventoryGatedAction(message, status) {
    if (!message) {
        return [];
    }
    const normalizedMessage = message.toLowerCase();
    const inventoryTokens = buildInventoryTokenSet(status);
    const hasAnyToken = (tokens) => tokens.some((token) => hasInventoryTokenMatch(inventoryTokens, token));
    const missing = [];
    const gatedChecks = [
        {
...
function detectHardImpossibleActionGates(message, status) {
    if (!message) {
        return [];
    }
    const normalizedMessage = message.toLowerCase();
    ...
    if (quotedItem && /(꺼내|사용|장착|equip|use|wield|draw|brandish)/i.test(message)) {
        const demandedItem = quotedItem[1]?.trim() || "";
        if (demandedItem && !hasAnyToken([demandedItem])) {
            hardGates.push("Quoted item '" + demandedItem + "' is not present in carried/equipped inventory; classify original action as impossible.");
        }
    }
    ...
    if (preResolvedPatterns.some((pattern) => pattern.test(normalizedMessage))) {
        hardGates.push("Completed-outcome assertions require prior in-scene evidence; do not narrate the claimed success as already true.");
    }
    return uniqStrings(hardGates).slice(0, 4);
}
function actionLikelyTargetsNpc(message, npc) {
...
function isNpcMemoryRelevantAction(message) {
    if (!message) {
        return false;
    }
    return /(질문|묻|심문|추궁|회유|협상|탐문|정보|단서|설득|협박|deal|question|ask|interrogat|negotiat|probe)/i.test(message);
}
async function updateAndBuildNpcMemoryChunk(params) {
    ...
    const summaries = visibleNpcs.map((npc) => {
        const npcMemory = toObject(byNpc[npc.id]);
...
function parseFastWaitDurationLabel(message) {
    const lower = message.toLowerCase();
...
function isFastWaitIntent(message) {
    if (!message) {
        return false;
    }
...
async function applyFastWaitWorldDrift(params) {
    ...
    const travelRoot = toObject(travelLoaded.parsed);
    const sceneRoot = toObject(sceneLoaded.parsed);
    const currentZoneId = normalizeZoneId(readString(toObject(travelRoot.travel_state).current_zone) ||
...
function clipForGuard(value, maxLength) {
    if (value.length <= maxLength) {
        return value;
    }
    return `${value.slice(0, maxLength)}...`;
}
function summarizeSceneClock(clock) {
...
async function buildActionFeasibilityGuardChunk(params) {
    const latestAction = extractLatestUserMessageFromPrompt(params.prompt) || extractLatestUserMessage(params.messages);
    ...
    const redact = (value) => redactHiddenNpcNames(value, npcVisibility);
    const statusData = params.statusPanelData ??
        (await loadStatusPanelData({
            cfg: params.cfg,
            worldRoot: params.worldRoot,
            seedBase: latestAction || "runtime",
        }));
...
function readDisclosureStage(value) {
    if (typeof value === "number" && Number.isFinite(value)) {
...
function detectScenePersistenceSignals(message) {
    const normalized = message.toLowerCase();
    return {
...
async function runCharacterBootstrapGate(params) {
    ...
    const scene = toObject(sceneRoot.scene);
    const sceneFlow = toObject(scene.scene_flow);
    const resolvedBefore = resolveRuntimePhase({
        playerCanonParsed: root,
        sceneParsed: sceneRoot,
    });
    const latestUserMessage = extractLatestUserMessageFromPrompt(params.prompt) || extractLatestUserMessage(params.messages);
    let changed = false;
    if (!isInGamePhase(resolvedBefore.phase)) {
        const bootstrapUpdate = parseBootstrapUpdate(latestUserMessage);
        for (const field of ["name", "background", "motive", "secret", "fear", "goal"]) {
        ...
        const mergedFreeform = mergeFreeformDescription(readString(player.freeform_description), incomingFreeform);
    ...
    if (characterCreated && !readString(player.secret)) {
        player.secret = deriveSystemSecret(player);
        changed = true;
    }
    ...
    if (changed) {
        ...
        if (!persisted.ok) {
            const message = persisted.error || "bootstrap player persistence failed";
            if (!isCanonicalWriteBackDisabledMessage(message)) {
    ...
    const inferredMerged = Array.from(new Set([...existingSeedStructures, ...toStringArray(worldHints.inferred_structures)]));
    const mergedSeedZones = Array.from(new Set(existingSeedZones));
    let worldSeedsChanged = false;
    if (inferredMerged.length !== existingSeedStructures.length ||
    ...
    if (worldSeedsChanged || relationshipsChanged) {
        ...
        if (!persisted.ok) {
            const message = persisted.error || "bootstrap seed/relationship persistence failed";
            if (!isCanonicalWriteBackDisabledMessage(message)) {
                throw new Error(message);
            }
        }
    }
    const resolved = resolveRuntimePhase({
        playerCanonParsed: root,
        sceneParsed: sceneRoot,
    });
    if (resolved.characterCreated) {
        await ensureStatusAndInventorySeed({
            cfg: params.cfg,
            worldRoot: params.worldRoot,
            seedBase: readString(player.name) || "player",
        });
    }
    if (isInGamePhase(resolved.phase)) {
        return {
            runtimePhase: "IN_GAME",
            characterCreated: resolved.characterCreated,
            bootstrapComplete: resolved.bootstrapComplete,
            justCompleted,
        };
    }
    const missingFields = collectMissingBootstrapFields(player);
    if (resolved.phase === "BOOTSTRAP") {
        return {
            ...
            contextChunk: joinLines([
                ...
                "character_created=false. Show character setup only.",
                "For this response, output order is mandatory:",
                "1) 캐릭터 생성/불러오기 안내",
                "2) 선택지: 새 캐릭터 시작 / 캐릭터 불러오기 / 초기 정보 유지(가능하면)",
                "3) 간단한 다음 단계 안내",
                "Do NOT output scene narration, travel, combat, state panel, or freeform game action flow.",
            ]),
        };
    }
    const contextLines = [
        "[TRPG_RUNTIME_CHARACTER_BOOTSTRAP]",
        "character_created=true AND bootstrap_complete=false. Bootstrap phase.",
        "GUIDE: Set up the player character step-by-step.",
        "For NAME (이름): Ask conversationally so they reply via chat.",
        "For BACKGROUND, MOTIVE, FEAR, GOAL: You MUST provide 3 to 4 creative options using a Dropdown Select Menu ('choices' array with scene: 'choice').",
        "Do NOT ask them to type out their background or Motives! Only let them choose from the Dropdown.",
        "Never mention hidden secrets or use rigid labels like 'PART A'.",
        "Focus on ONE missing field per turn.",
        "Even if user says '게임 시작', remain in the bootstrap choice flow until all fields are collected.",
    ];
    if (readString(player.name))
        contextLines.push(`Known 이름: ${readString(player.name)}`);
    if (readString(player.background))
        contextLines.push(`Known 출신 / 배경: ${readString(player.background)}`);
    if (readString(player.motive))
        contextLines.push(`Known 지금 이 세계에 들어온 이유: ${readString(player.motive)}`);
    if (readString(player.fear))
        contextLines.push(`Known 두려워하는 것: ${readString(player.fear)}`);
    if (readString(player.goal))
        contextLines.push(`Known 지금 당장의 목표: ${readString(player.goal)}`);
...
function isAllowedRuntimeAgent(cfg, agentId) {
    ...
    register(api) {
        ...
        api.on("before_prompt_build", async (event, hookCtx) => {
            ...
            try {
                ...
                const extractedLatestAction = extractLatestUserMessageFromPrompt(event.prompt) || extractLatestUserMessage(promptMessages);
                const latestAction = extractedLatestAction ||
                ...
                if (!isInGamePhase(bootstrap.runtimePhase)) {
                    appendChunks.push(NON_GAME_COMPONENT_USAGE_GUIDE);
                    api.logger.info("[trpg-runtime] non-game phase active phase=" + bootstrap.runtimePhase);
                    ...
                    if (bootstrapBudgeted.droppedTags.length > 0) {
                        api.logger.info("[trpg-runtime] injection budget dropped tags=" + bootstrapBudgeted.droppedTags.join(","));
                    }
                    return {
                ...
                if (guard.introRequired) {
                    ...
                    api.logger.info("[trpg-runtime] intro guard applied for scene " + guard.sceneId + "; intro_shown toggled true");
                }
                ...
                // Discord component usage guide — game phase only
                appendChunks.push(GAME_COMPONENT_USAGE_GUIDE);
                ...
                if (transition.shouldTick) {
                    ...
                    api.logger.info("[trpg-runtime] faction tick preview trigger=scene_transition reason=" +
                        transition.reason +
                        " advanced=" +
                ...
                if (budgeted.droppedTags.length > 0) {
                    api.logger.info("[trpg-runtime] injection budget dropped tags=" + budgeted.droppedTags.join(","));
                }
                return {
                ...
                api.logger.warn(`[trpg-runtime] prompt hook skipped: ${error instanceof Error ? error.message : String(error)}`);
                return;
            }
        });
        api.registerTool((ctx) => ({
            name: "trpg_store_get",
            description: "Read structured TRPG world data by entity id, path, or scope with explicit knowledge-view filtering.",
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Validate a TRPG patch proposal without writing files and return conflicts plus normalized diff preview.",
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Advance or preview causality-first offscreen faction motion with drop/delay/silent emission summaries.",
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Apply a previously validated TRPG patch only with canon-auditor approval metadata and strict world-root write guards.",
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Build lifecycle compaction patch drafts with weighted pruning candidates and optional audited apply.",
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Return dormant hook and reveal candidates with prerequisite status and tension scoring.",
        ...
        const SCENE_COMPONENT_PARAMETERS = {
            ...
            properties: {
                ...
                combat: {
                    ...
                    required: ["round", "ac", "enemySummary"],
                },
                buttons: {
                    type: "array",
                    items: {
                ...
                imageUrl: {
                    type: "string",
                    description: "Optional Image URL to display in the embed",
                },
                /*        inventoryChoices: {
                          type: "array",
                          items: {
                          ...
                          description: "Unified Context: List of inventory items to render as a dropdown menu",
                        },*/
            },
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Build a Discord component payload for a TRPG scene response. Returns JSON components to pass to the message tool. " +
            ...
            async execute(_toolCallId, params) {
                ...
                try {
                    const status = await loadStatusPanelData({ cfg, worldRoot: gate.worldRoot, seedBase: "player" });
                    const [playerLoaded, sceneLoaded] = await Promise.all([
                        loadStructuredWorldFile(gate.worldRoot, "canon/player.yaml", {
                            allowMissing: true,
                            maxReadBytes: cfg.maxReadBytes,
                        }),
                        loadStructuredWorldFile(gate.worldRoot, "state/current-scene.yaml", {
                            allowMissing: true,
                            maxReadBytes: cfg.maxReadBytes,
                        }),
                    ]);
                    const resolvedPhase = resolveRuntimePhase({
                        playerCanonParsed: playerLoaded.parsed,
                        sceneParsed: sceneLoaded.parsed,
                    });
                    const inGame = isInGamePhase(resolvedPhase.phase);
                    const sceneInput = { ...params };
                    if (!inGame) {
                        Object.assign(sceneInput, normalizeSceneInputForNonGame(sceneInput));
                    }
                    if (sceneInput.scene === "combat" && sceneInput.combat) {
                        sceneInput.combat.hpCurrent = status.hpCurrent;
                        sceneInput.combat.hpMax = status.hpMax;
                        sceneInput.combat.manaCurrent = status.staminaCurrent;
                        sceneInput.combat.manaMax = status.staminaMax;
                    }
                    if (inGame && status.carriedItems.length > 0) {
                        sceneInput.inventoryChoices = status.carriedItems.map((item) => ({
                            label: item,
                            value: `use_${item.replace(/\s+/g, '_')}`,
                            emoji: "🎒"
                        }));
                    }
                    if (sceneInput.includeInput === undefined) {
                        sceneInput.includeInput = true;
                    }
                    const componentsRaw = buildSceneComponents(sceneInput);
                    const components = inGame ? componentsRaw : normalizeBuiltComponentsForNonGame(componentsRaw);
                    return jsonToolResult({
                        ok: true,
                        runtimePhase: resolvedPhase.phase,
                        components,
        ...
        api.logger.info("[trpg-runtime] registered tools: trpg_store_get, trpg_patch_dry_run, trpg_patch_apply, trpg_state_compact, trpg_faction_tick, trpg_hooks_query, trpg_dice_roll, trpg_scene_components");
    },
};
