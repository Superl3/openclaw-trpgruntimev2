...
import { runPatchDryRun, runPatchApply, } from "./patch-engine.js";
...
function buildZoneGraph(parsedPressure) {
    ...
    const out = {};
    const upsert = (rawNode, fallbackId = "") => {
        ...
        out[zoneId] = {
            ...
            connections,
            pressure: readFiniteNumber(rawNode.pressure) ?? readFiniteNumber(rawNode.score) ?? existing?.pressure ?? null,
        };
    };
...
function countRelationshipThreadsByZone(relationshipsRoot) {
    const result = {};
...
function collectVisibleNpcIds(sceneRoot) {
    const out = new Set();
...
export async function buildLifecycleCompactionPlan(params) {
    const [pressureLoaded, sceneLoaded, travelLoaded, worldSeedsLoaded, factionLoaded, relationshipsLoaded, memoryLoaded, eventsLoaded, compactionLoaded, archiveLoaded,] = await Promise.all([
    ...
    const compactionRoot = Object.keys(toObject(compactionLoaded.parsed)).length > 0
    ...
    const archiveRoot = Object.keys(toObject(archiveLoaded.parsed)).length > 0
    ...
    const usedArchiveSummaryIds = new Set();
    ...
    for (const [zoneId, zone] of Object.entries(zoneGraph)) {
        ...
        const significance = readFiniteNumber(existing.significance_score) ??
            Math.max(8, Math.min(100, zoneTypeSignificance(zone.type) +
                Math.min(12, zone.tags.length * 2) +
                (isProtected ? 20 : 0) +
                Math.min(18, activeThreads * 4) +
                (hasFactionLink ? 8 : 0)));
        const witnessedRecently = daysSinceIso(lastPresence, nowMs) < 28;
        const retentionWeight = readFiniteNumber(existing.retention_weight) ??
        ...
        if (JSON.stringify(existing) !== JSON.stringify(lifecycleNode)) {
            zoneLifecycleMap[zoneId] = lifecycleNode;
            stateChanged = true;
        }
    }
    ...
    if (history.length > 24 && candidates.length < params.maxCandidates) {
        ...
        archiveSummaries.push(buildArchiveSummaryEntry({
