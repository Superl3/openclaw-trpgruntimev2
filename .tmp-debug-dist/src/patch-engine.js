...
import { loadStructuredWorldFile, normalizeWorldRelativePath, renderStructuredContent, resolveWorldAbsolutePath, supportsStructuredPatchPath, } from "./world-store.js";
...
function parseArrayIndex(token) {
    if (!/^\d+$/.test(token)) {
        return null;
    }
...
function normalizePatchPayload(input, cfg) {
    ...
    if (operationsRaw.length > cfg.maxOperationsPerPatch) {
        throw new Error(`operations exceed maxOperationsPerPatch (${cfg.maxOperationsPerPatch}); received ${operationsRaw.length}`);
    }
    const operations = operationsRaw.map((entry, idx) => {
        ...
        const expectedSha256 = typeof opObj.expectedSha256 === "string" && opObj.expectedSha256.trim()
    ...
    const digest = hashSha256(JSON.stringify({
        title,
        allowNewFiles,
        operations,
    }));
    const patchId = typeof raw.patchId === "string" && raw.patchId.trim() ? raw.patchId.trim() : `trpg-${digest.slice(0, 12)}`;
    return {
        patchId,
        title,
        allowNewFiles,
...
export function createPatchCache() {
    return new Map();
}
function buildRollbackHint(simulation) {
    const operations = simulation.touchedFiles.map((file) => {
        const state = simulation.fileStates.get(file);
        return {
            op: "set",
            file,
            pointer: "/",
...
function parseAuditedApplyGate(input) {
    const audit = toObject(input.audit);
    if (Object.keys(audit).length === 0) {
        return {
            ok: false,
            error: "audited apply requires audit metadata (approvedBy=canon-auditor, verdict=pass, conflictStatus=non-conflicting)",
        };
    }
    ...
    const canonAbsorptionVerdict = typeof audit.canonAbsorptionVerdict === "string" ? audit.canonAbsorptionVerdict.trim() : undefined;
...
function isAuditedWritableScope(file) {
    return file.startsWith("state/") || file.startsWith("canon/");
}
function isCanonicalTarget(file) {
