...
const TERMINAL_LIFECYCLE = new Set(["resolved", "failed"]);
...
function anchorTitle(archetype) {
    switch (archetype) {
        case "smuggling":
            return "밀수 축";
        case "outbreak":
...
function normalizeAnchor(value, nowIso) {
    ...
    return {
        ...
        linkedQuestIds: Array.isArray(node.linkedQuestIds)
            ? uniqStrings(node.linkedQuestIds.filter((entry) => typeof entry === "string"), MAX_LINKED_QUESTS)
            : [],
        recentEventRefs: Array.isArray(node.recentEventRefs)
            ? uniqStrings(node.recentEventRefs.filter((entry) => typeof entry === "string"), MAX_EVENT_REFS)
            : [],
        sourceRefs: Array.isArray(node.sourceRefs)
            ? uniqStrings(node.sourceRefs.filter((entry) => typeof entry === "string"), MAX_SOURCE_REFS)
            : [],
        escalationCount: Math.max(0, readInt(node.escalationCount, 0)),
    };
}
function makeDefaultAnchorState() {
    return {
        version: 1,
        anchors: [],
        nextAnchorSeq: 1,
...
function compareAnchorPriority(a, b) {
    const rank = (value) => {
        switch (value) {
            case "escalated":
                return 6;
...
function parseSignal(input) {
    ...
    const markEscalatedAnchorIds = Array.isArray(node.markEscalatedAnchorIds)
        ? uniqStrings(node.markEscalatedAnchorIds.filter((entry) => typeof entry === "string"), 16)
        : [];
    const markResolvedAnchorIds = Array.isArray(node.markResolvedAnchorIds)
        ? uniqStrings(node.markResolvedAnchorIds.filter((entry) => typeof entry === "string"), 16)
        : [];
    const hasKnownField = Array.isArray(node.pressureBoostById) || Array.isArray(node.markEscalatedAnchorIds) || Array.isArray(node.markResolvedAnchorIds);
    if (!hasKnownField) {
        return {
            mode: "invalid",
...
function transitionLifecycle(anchor, next, nowIso, reason) {
    const nextState = {
        ...anchor,
        lifecycle: next,
...
function transitionsByQuestSummary(summary) {
    const out = new Map();
...
export function ensureAnchorRuntimeState(value) {
...
function getPressureIntensityById(economy) {
    const out = new Map();
...
function linkedQuestIdsForPressure(economy, pressureId) {
    return uniqStrings(economy.quests
        .filter((quest) => quest.pressureId === pressureId && (quest.lifecycle === "active" || quest.lifecycle === "stalled" || quest.lifecycle === "surfaced"))
        .map((quest) => quest.questId), MAX_LINKED_QUESTS);
}
function isStarted(anchor) {
...
function capAnchorList(anchors) {
...
export function runAnchorTick(input) {
    ...
    const existingPressureIds = new Set(anchors
    ...
    anchors = anchors.map((anchor) => {
        ...
        if (anchor.lifecycle === "active") {
            const shouldEscalate = escalateBySignal.has(anchor.anchorId) ||
