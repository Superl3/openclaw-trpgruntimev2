...
import { accumulateBehavioralDrift, buildIntentAnalyzerInput, buildPersonaDriftAnalyzerInput, deterministicActionFromFreeInput, rememberFreeInputTrace, selectStructuredActionIntent, updateIntentInertia, validateIntentAnalyzerOutput, validatePersonaDriftAnalyzerOutput, } from "./analyzer-lane.js";
import { PANEL_MODAL_SUBMIT_ACTION_ID, collectPanelRouteActionIds } from "./panel-mvp.js";
import { createInitialDeterministicSceneLoop, ensureDeterministicSceneLoopState, feasibilityLabel, resolveDeterministicSceneAction, zeroBehavioralAxisVector, } from "./scene-loop.js";
import { appendTraceEvent, createTraceEvent, ensureTraceState } from "./trace.js";
import { applyQuestHookTextOverrides, buildQuestHookSlotSourceHash, isQuestHookTextCacheValid, setQuestHookTextDebugState, } from "./quest-economy.js";
import { RUNTIME_SCHEMA_VERSION, ensureSessionPresentationState, ensureRuntimeMetadata, } from "./types.js";
import { normalizeRuntimeSafetyFlags, } from "./safety-flags.js";
const DEFAULT_SCENE_ID = "scene-bootstrap";
const PANEL_IDS = ["fixed", "main", "sub"];
const DEFAULT_HOOK_TEXT_TIMEOUT_MS = 350;
const DEFAULT_HOOK_TEXT_CACHE_TTL_SEC = 900;
const NOOP_HOOK_TEXT_RENDERER = {
    async render() {
        return {
...
function nextUiVersion(value) {
    if (!Number.isFinite(value) || value < 1) {
        return 1;
    }
    return Math.trunc(value) + 1;
}
function nextActionSeq(currentActionSeq, legacyTurnIndex) {
...
function pressureIntensityBand(value) {
...
function anchorEventTypeToTraceType(eventType) {
    switch (eventType) {
        case "formed":
...
class Checkpoint0RuntimeEngine {
    store;
    intentAnalyzer;
    ...
    constructor(deps) {
        ...
        this.runtimeSafetyFlags = normalizeRuntimeSafetyFlags({
            ...
            richHookActionableEnabled: typeof deps.runtimeSafetyFlags?.richHookActionableEnabled === "boolean"
                ? deps.runtimeSafetyFlags.richHookActionableEnabled
                : hasLegacyRichHookTextEnabled
                    ? legacyRichHookTextEnabled
                    : undefined,
            richHookWorldPulseEnabled: typeof deps.runtimeSafetyFlags?.richHookWorldPulseEnabled === "boolean"
                ? deps.runtimeSafetyFlags.richHookWorldPulseEnabled
                : hasLegacyRichHookTextEnabled
                    ? legacyRichHookTextEnabled
                    : undefined,
        });
        this.hookTextTimeoutMs = Number.isFinite(deps.hookTextTimeoutMs)
            ? Math.max(80, Math.min(2_000, Math.trunc(deps.hookTextTimeoutMs)))
            : DEFAULT_HOOK_TEXT_TIMEOUT_MS;
        this.hookTextCacheTtlSec = Number.isFinite(deps.hookTextCacheTtlSec)
            ? Math.max(60, Math.min(7_200, Math.trunc(deps.hookTextCacheTtlSec)))
            : DEFAULT_HOOK_TEXT_CACHE_TTL_SEC;
        this.traceMaxEvents = Number.isFinite(deps.traceMaxEvents)
            ? Math.max(20, Math.min(500, Math.trunc(deps.traceMaxEvents)))
            : 120;
        this.analyzerMemoryTtlSec = Number.isFinite(deps.analyzerMemoryTtlSec)
            ? Math.max(60, Math.min(86_400, Math.trunc(deps.analyzerMemoryTtlSec)))
            : 900;
        this.clock = deps.clock ?? new SystemClock();
        this.idGenerator = deps.idGenerator ?? new RuntimeIdGenerator();
    }
    normalizeSessionLoop(session, nowIso) {
        const loop = ensureDeterministicSceneLoopState(session.deterministicLoop, {
            sceneId: session.sceneId,
            nowIso,
        });
        const actionSeq = Math.max(0, Number.isFinite(session.actionSeq)
            ? Math.trunc(session.actionSeq)
            : 0, Number.isFinite(session.turnIndex)
            ? Math.trunc(session.turnIndex)
            : 0);
        const committedDispatchIds = session.panelDispatch && Array.isArray(session.panelDispatch.committedDispatchIds)
    ...
    async renderQuestHookTextWithTimeout(input) {
        let timer = null;
    ...
    async registerDefaultPanelRoutes(session) {
        const out = [];
        const routeActionIds = collectPanelRouteActionIds(session);
        const availabilityByAction = new Map(session.deterministicLoop.actionPalette.map((entry) => [entry.actionId, entry.availability]));
        for (const actionId of routeActionIds) {
            out.push(await this.bindInteractionRoute({
    ...
    async startNewSession(input) {
        ...
        if (existingSession) {
            ...
            const endedSessionBase = {
                ...normalizedExisting,
                status: "ended",
                updatedAt: nowIso,
                endedAt: nowIso,
            };
            const endedSession = appendTraceEvent(endedSessionBase, createTraceEvent({
                lane: "engine",
                type: "session.end",
                tsIso: nowIso,
                data: {
        ...
        const session = appendTraceEvent(sessionBase, createTraceEvent({
            lane: "engine",
            type: "session.new",
            tsIso: nowIso,
            data: {
    ...
    async resumeSession(input) {
        ...
        const byChannel = !byId && input.channelKey ? await this.store.readActiveSessionByChannel(readNonEmptyString(input.channelKey, "")) : null;
        ...
        const nextSession = appendTraceEvent(nextSessionBase, createTraceEvent({
            lane: "engine",
            type: "session.resume",
            tsIso: nowIso,
            data: {
        ...
        const recoveryPlan = PANEL_IDS.map((panelId) => {
    ...
    async bindInteractionRoute(input) {
        const nowIso = this.clock.nowIso();
    ...
    async resolveInteractionRoute(input) {
        const routeKey = {
            sessionId: readNonEmptyString(input.sessionId, ""),
            uiVersion: Math.max(1, Math.trunc(input.uiVersion)),
            sceneId: readNonEmptyString(input.sceneId, DEFAULT_SCENE_ID),
    ...
    async processSceneAction(input) {
        const nowIso = this.clock.nowIso();
        ...
        let session = appendTraceEvent(sessionBase, createTraceEvent({
            lane: "engine",
        ...
        if (isFreeSentenceInput) {
            ...
            selectedActionId = readNonEmptyString(selected.actionId, "action.unknown");
        ...
        let hookTextResult = "skipped";
        let hookTextReason = null;
        ...
        const stripExpiredHookSlotCache = (slot) => ({
            ...slot,
            llmShortText: null,
            llmSourceHash: null,
        ...
        const cacheHitBySlotKey = new Map(cacheStates.filter((entry) => entry.cacheHit).map((entry) => [entry.slot.slotKey, true]));
        ...
        const finalSlotRows = finalHookSlots.map((slot) => ({
            slot,
            slotType: "actionable",
        }));
    ...
    async analyzePersonaDrift(input) {
        return this.personaDriftAnalyzer.analyze(input);
    }
    async renderQuestHookText(input) {
        return this.questHookTextRenderer.render(input);
    }
    async renderScene(input) {
        return this.sceneRenderer.render(input);
    }
}
export function createCheckpoint0RuntimeEngine(deps) {
