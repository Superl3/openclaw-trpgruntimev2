...
function decayToward(value, target, step) {
...
function classifyResidue(sentiment) {
    if (sentiment >= 25) {
        return "friendly";
    }
    if (sentiment <= -45) {
...
function normalizeLocationTemporalState(value, nowIso) {
    ...
    const incidents = Array.isArray(node.recentIncidents)
        ? uniqStrings(node.recentIncidents.filter((entry) => typeof entry === "string"), MAX_INCIDENTS_PER_LOCATION)
        : [];
    return {
        locationId,
...
function normalizeLocationBaseline(value, nowIso) {
    const node = toRecord(value);
    const locationId = normalizeLocationId(node.locationId);
    if (!locationId) {
        return null;
...
function normalizeBootstrapLocationStates(value, nowIso) {
...
function normalizeNpcMemoryState(value, nowIso) {
    const node = toRecord(value);
    ...
    const emotionalResidue = residueRaw === "friendly" || residueRaw === "tense" || residueRaw === "hostile" || residueRaw === "neutral"
        ? residueRaw
        : fallbackResidue;
    const impressionTags = Array.isArray(node.impressionTags)
        ? uniqStrings(node.impressionTags.filter((entry) => typeof entry === "string"), MAX_IMPRESSION_TAGS)
        : [];
    return {
        npcId,
...
function normalizeInfoFreshnessState(value, nowIso) {
    const node = toRecord(value);
    ...
    const sourceType = sourceRaw === "dialogue" || sourceRaw === "rumor" || sourceRaw === "witness" ? sourceRaw : "observation";
...
function normalizeResidualTraceState(value, nowIso) {
    const node = toRecord(value);
    const traceTypeRaw = readString(node.traceType);
    const traceType = traceTypeRaw === "noise" ||
        traceTypeRaw === "alarm" ||
...
function pruneTemporalState(state) {
...
export function ensureTemporalRuntimeState(value, nowIso, bootstrap) {
    ...
    const normalized = {
        version: 1,
        locationStates: locationStatesRaw.length > 0
            ? locationStatesRaw
...
function traceIncidentLabel(traceType) {
    switch (traceType) {
        case "noise":
...
function upsertResidualTrace(params) {
    ...
    const index = params.traces.findIndex((entry) => entry.traceType === params.traceType && entry.locationId === params.locationId);
    if (index < 0) {
        return {
            traces: [
                ...params.traces,
    ...
    const target = params.traces[index];
...
function upsertInfoFreshness(params) {
    ...
    const current = params.freshness[index];
...
function upsertNpcMemory(params) {
    const index = params.memory.findIndex((entry) => entry.npcId === params.npcId && entry.locationId === params.locationId);
    if (index < 0) {
    ...
    const current = params.memory[index];
...
function applyActionFootprint(params) {
    ...
    let tracesUpdated = 0;
    const incidentNotes = [];
    const addTrace = (traceType, amount) => {
...
function applyLocationProjection(params) {
    ...
    const targetTension = clampInt(Math.round(22 + totals.alarm * 0.58 + totals.disorder * 0.4 + totals.witness * 0.25 + totals.noise * 0.16), 0, 100);
    const targetAlertness = clampInt(Math.round(16 + totals.alarm * 0.68 + totals.witness * 0.44 + totals.noise * 0.19), 0, 100);
    const targetAccessibility = clampInt(Math.round(82 - targetAlertness * 0.3 - totals.disorder * 0.2 + totals.opened_path * 0.25), 0, 100);
    const existingIndex = params.temporal.locationStates.findIndex((entry) => entry.locationId === params.locationId);
    const existing = existingIndex >= 0
        ? params.temporal.locationStates[existingIndex]
        : {
            locationId: params.locationId,
            tension: 35,
            alertness: 30,
    ...
    const locationShifted = nextState.tension !== existing.tension ||
...
export function buildTemporalQualitativeSummary(params) {
    ...
    const memory = maxFamiliarity >= 70
        ? "NPC memory is vivid."
        : maxFamiliarity >= 35
    ...
    const freshness = maxFreshness >= 70
    ...
    const traces = maxTraceIntensity >= 65
...
export function runTemporalUpdatePipeline(input) {
