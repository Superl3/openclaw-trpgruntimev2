import { LLM_CONTRACT_VERSION, emptyDriftAxisVector, isIntentAnalyzerOutput, isPersonaDriftAnalyzerOutput, } from "./llm-contracts.js";
...
function chooseConservativeFallback(availableActions, lastMappedActionId) {
...
function promptEnvelope(task, input) {
    return [
        `TASK=${task}`,
...
function scoreToIntentLabel(text) {
...
function detectPreResolvedClaim(text) {
    const lower = text.toLowerCase();
...
function scoreActionCandidates(text, allowedActions) {
    const lower = text.toLowerCase();
    const scores = new Map();
    const addScore = (actionId, delta) => {
        if (!allowedActions.includes(actionId)) {
            return;
        }
        scores.set(actionId, clamp((scores.get(actionId) ?? 0) + delta, 0, 1));
    };
    if (/(조사|살핀|탐색|inspect|observe|look)/.test(lower))
        addScore("action.observe", 0.8);
    if (/(이동|간다|전진|advance|move|travel)/.test(lower))
        addScore("action.move", 0.85);
    if (/(대기|기다|멈추|hold|wait)/.test(lower))
        addScore("action.wait", 0.8);
    if (/(대화|말|질문|talk|ask|speak)/.test(lower))
        addScore("action.talk", 0.82);
    if (/(강행|돌파|rush|force|push)/.test(lower))
        addScore("action.rush", 0.9);
    if (scores.size === 0) {
...
function heuristicIntentOutput(input) {
...
function axisDeltaTemplate() {
    return emptyDriftAxisVector();
}
function addAxis(delta, axis, amount) {
    delta[axis] = clamp(delta[axis] + amount, -1, 1);
}
function heuristicDriftOutput(input) {
...
export function validateIntentAnalyzerOutput(value) {
    return isIntentAnalyzerOutput(value) ? value : null;
}
export function validatePersonaDriftAnalyzerOutput(value) {
...
export function buildPersonaDriftAnalyzerInput(params) {
    const loop = params.session.deterministicLoop;
...
export function selectStructuredActionIntent(params) {
    ...
    const deterministicCandidate = params.deterministicActionId !== "action.unknown" && available.has(params.deterministicActionId)
    ...
    let fallbackStrategy = conservative.strategy;
    ...
    const source = analyzerWeight >= 0.2 && analyzer !== null && analyzer.confidence >= INTENT_CONFIDENCE_HIGH && selectedActionId !== deterministicCandidate
        ? "analyzer"
        : "deterministic";
...
export function updateIntentInertia(params) {
    const streakCount = params.current.lastMappedActionId && params.current.lastMappedActionId === params.selectedActionId
...
export function accumulateBehavioralDrift(params) {
    ...
    return {
        ...
        drift: {
            ...
            boldness: clamp(params.current.drift.boldness * DRIFT_DECAY + params.analyzerOutput.driftDelta.boldness * gain, -1, 1),
            caution: clamp(params.current.drift.caution * DRIFT_DECAY + params.analyzerOutput.driftDelta.caution * gain, -1, 1),
            altruism: clamp(params.current.drift.altruism * DRIFT_DECAY + params.analyzerOutput.driftDelta.altruism * gain, -1, 1),
            aggression: clamp(params.current.drift.aggression * DRIFT_DECAY + params.analyzerOutput.driftDelta.aggression * gain, -1, 1),
...
export class RuleBasedPersonaDriftAnalyzer {
    invoker;
    constructor(invoker) {
        this.invoker = invoker;
    }
...
export function deterministicActionFromFreeInput(text) {
    return mapFreeInputToActionDeterministic(text);
}
export function freezeCoreIdentity(core) {
    return {
