...
function normalizeLocationVolatility(value) {
    const normalized = readString(value);
...
function normalizeLocationSeed(value, index) {
    const node = toRecord(value);
...
function normalizePressureSeed(value, index) {
    const node = toRecord(value);
    return {
...
function normalizeFactionSeed(value, index) {
    const node = toRecord(value);
    return {
...
function normalizeNpcArchetypePool(value, index) {
    const node = toRecord(value);
    const factionId = readString(node.factionId);
    return {
        npcArchetypeId: readString(node.npcArchetypeId) ||
...
function ensureIsoOrFallback(value) {
...
export function extractWorldSeedEnvelope(value) {
...
export function validateWorldSeed(value) {
    const issues = [];
    ...
    const locations = (Array.isArray(root.locations) ? root.locations : []).map((entry, index) => normalizeLocationSeed(entry, index));
    const pressures = (Array.isArray(root.pressures) ? root.pressures : []).map((entry, index) => normalizePressureSeed(entry, index));
    const factions = (Array.isArray(root.factions) ? root.factions : []).map((entry, index) => normalizeFactionSeed(entry, index));
    const npcPool = (Array.isArray(root.npcPool) ? root.npcPool : []).map((entry, index) => normalizeNpcArchetypePool(entry, index));
...
function locationVolatilityDelta(value) {
    if (value === "stable") {
        return {
            tension: -4,
            alertness: -4,
...
function normalizeDeterminismKey(seed) {
...
export function buildWorldSeedFingerprint(seed) {
    return hashSha256(stableStringify({
        schemaVersion: seed.schemaVersion,
...
export function buildRuntimeBootstrapInput(seed) {
