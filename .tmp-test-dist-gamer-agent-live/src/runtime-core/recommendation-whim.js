export const DEFAULT_WHIM_CONFIG = {
    enabled: false,
...
function deterministicUnitFromActionId(actionId) {
    let hash = 2166136261;
...
function resolveWhimConfig(config) {
    return {
        enabled: config?.enabled ?? DEFAULT_WHIM_CONFIG.enabled,
        positiveSurpriseWeight: clamp(config?.positiveSurpriseWeight ?? DEFAULT_WHIM_CONFIG.positiveSurpriseWeight, 0, 0.2),
        negativeSurpriseWeight: clamp(config?.negativeSurpriseWeight ?? DEFAULT_WHIM_CONFIG.negativeSurpriseWeight, 0, 0.2),
        maxAbsoluteAdjustment: clamp(config?.maxAbsoluteAdjustment ?? DEFAULT_WHIM_CONFIG.maxAbsoluteAdjustment, 0, 0.3),
        ...
        highRiskGlobalScale: clamp(config?.highRiskGlobalScale ?? DEFAULT_WHIM_CONFIG.highRiskGlobalScale, 0, 1),
        highRiskRecklessScale: clamp(config?.highRiskRecklessScale ?? DEFAULT_WHIM_CONFIG.highRiskRecklessScale, 0, 1),
    };
}
export function applyWhimAdjustment(input) {
    ...
    const taper = gap <= config.taperStartGap
        ? 1
    ...
    const adjustment = clamp(baseline * taper * safetyScale, -config.maxAbsoluteAdjustment, config.maxAbsoluteAdjustment);
    return {
        enabled: true,
