...
const LIVE_LIFECYCLE_SET = new Set(["seed", "surfaced", "active", "stalled"]);
const VISIBLE_LIFECYCLE_SET = new Set(["surfaced", "active", "stalled"]);
const TERMINAL_LIFECYCLE_SET = new Set(["resolved", "failed", "deleted"]);
...
function readIsoString(value, fallback) {
...
function isoNowOrFallback(nowIso) {
...
function elapsedSec(fromIso, toIso) {
...
function normalizeQuestCost(value, fallback) {
...
function pressureSeedCost(intensity) {
    if (intensity >= 78) {
...
function pressureLabel(archetype) {
    switch (archetype) {
        case "smuggling":
            return "smuggling";
        case "outbreak":
...
function compareQuestPriority(a, b) {
...
function makeDefaultPresentationState() {
    return {
        recentOutcomes: [],
        hookSlots: [],
        worldPulseSlot: null,
...
function makeDefaultPressures(nowIso) {
    return [
        {
...
function normalizeBootstrapPressure(value, nowIso) {
    const node = toRecord(value);
    ...
    return {
        ...
        targetLocations: Array.isArray(node.targetLocations)
            ? uniqStrings(node.targetLocations.filter((entry) => typeof entry === "string"), MAX_TARGET_LOCATIONS)
            : [],
...
function normalizeBootstrapPressures(value, nowIso) {
...
function makeDefaultEconomy(nowIso, bootstrap) {
...
function normalizePressure(value, nowIso) {
    const node = toRecord(value);
    const pressureId = readString(node.pressureId);
    if (!pressureId) {
        return null;
...
function normalizeQuest(value, params) {
    const node = toRecord(value);
...
function extractQuestSeq(value) {
...
function recomputeNextQuestSeq(quests, current) {
...
function dedupeQuestList(quests) {
    const map = new Map();
...
function pruneEconomy(state) {
    const dedupedPressures = new Map();
...
function normalizeSoftQuota(value, fallback) {
    const node = toRecord(value);
    const capsNode = toRecord(node.caps);
    const parseUsageRows = (raw) => {
...
function hookTextSourceFromSlot(slot) {
    return slot.llmShortText ? "llm" : "default";
}
function hookTextSlotTypeFromSlot(slot) {
    return slot.slotKey === WORLD_PULSE_HOOK_SLOT_KEY ? "worldPulse" : "actionable";
}
function emptyHookTextDebugState() {
    return {
...
export function buildQuestHookSlotSourceHash(slot) {
    return hashFNV32([
        slot.slotKey,
        slot.questId,
        slot.lifecycle,
...
function normalizeShortHookText(raw, defaultText) {
...
export function isQuestHookTextCacheValid(slot, nowIso) {
...
function sanitizeQuestHookSlotCache(slot, nowIso) {
...
function reconcileHookTextDebugState(state, params) {
    const previousMetaByKey = new Map(state.slotMeta.map((entry) => [entry.slotKey, entry]));
    const candidates = params.hookSlots.slice(0, MAX_HOOK_SLOTS);
...
function normalizeUrgencyBand(value) {
    const normalized = readString(value);
...
function normalizeRecentOutcome(value, fallbackNowIso) {
    const node = toRecord(value);
    const questId = readString(node.questId);
    if (!questId) {
        return null;
    ...
    const kind = kindRaw === "expired" || kindRaw === "failed" || kindRaw === "resolved" || kindRaw === "mutated"
        ? kindRaw
        : "failed";
    return {
...
function normalizeHookSlot(value) {
    const node = toRecord(value);
    ...
    const lifecycle = lifecycleRaw === "active" || lifecycleRaw === "stalled" || lifecycleRaw === "surfaced"
        ? lifecycleRaw
        : "surfaced";
...
function normalizeHookTextDebugSlot(value) {
    const node = toRecord(value);
    const slotKey = readString(node.slotKey);
    if (!slotKey) {
        return null;
    ...
    const slotType = slotTypeRaw === "worldPulse" ? "worldPulse" : "actionable";
    const sourceRaw = readString(node.source);
    const source = sourceRaw === "llm" ? "llm" : "default";
    return {
        slotKey,
        slotType,
        source,
...
function normalizeHookTextDebugState(value) {
    ...
    const result = resultRaw === "applied" || resultRaw === "fallback" || resultRaw === "skipped" ? resultRaw : "skipped";
    return {
...
function normalizeTuningSnapshot(value, fallback) {
    const node = toRecord(value);
...
function normalizeTuningSample(value, fallbackNowIso) {
    const node = toRecord(value);
    const budget = toRecord(node.budgetUtilization);
    const quota = toRecord(node.quotaSaturation);
    return {
...
function normalizePresentationState(value, fallbackNowIso) {
...
function usageMapsFromLiveQuests(quests) {
...
function computeBudgetUsed(quests) {
    const used = {
        livePool: 0,
        world: 0,
        attention: 0,
...
function recalculateBudgetAndQuota(state) {
...
export function ensureQuestEconomyState(value, nowIso, bootstrap) {
    const resolvedNow = isoNowOrFallback(nowIso);
...
function topPressure(pressures) {
...
function pressureArchetypeText(archetype) {
    switch (archetype) {
        case "smuggling":
            return "밀수";
        case "outbreak":
...
function locationText(locationId) {
...
function questSlotPriority(quest, locationId) {
...
function buildHookSlotFromQuest(params) {
    ...
    return {
        ...
        lifecycle: params.quest.lifecycle === "active" || params.quest.lifecycle === "stalled"
...
function deriveHookSlots(params) {
    ...
    if (activeTop) {
        slots.push(buildHookSlotFromQuest({
            quest: activeTop,
...
export function buildQuestEconomyQualitativeSummary(params) {
    ...
    const activeCount = params.economy.quests.filter((quest) => quest.lifecycle === "active" || quest.lifecycle === "stalled").length;
    const top = topPressure(params.economy.worldPressures);
    const hookSlots = params.economy.presentation.hookSlots.length > 0
    ...
    const actionableActiveText = activeTop
        ...
        : "진행 중 과제가 없다.";
    const actionableSurfacedText = surfacedTop.length > 0
...
function canAllocateSeed(params) {
    ...
    return (used.livePool + 1 <= params.caps.livePool &&
...
function shouldTryStart(actionId, classification) {
...
function progressDelta(params) {
    ...
    const base = params.actionId === "action.observe"
        ? 14
...
function deadlineSecondsFromUrgency(urgency) {
...
function adjustMomentum(current, delta) {
...
function hasExistingSuccessor(quests, parentQuestId) {
...
function averageUrgency(quests) {
...
function activeVsSurfacedRatio(quests) {
...
function budgetUtilization(budget) {
    return {
...
function transitionOutcomeKind(entry) {
    if (entry.to === "deleted") {
        return "expired";
    }
...
function updatePresentationState(params) {
    ...
    const carryWorldPulseLlm = previousWorldPulse &&
...
export function applyQuestHookTextOverrides(params) {
    ...
    const appliedSlotKeys = [];
    const ignoredSlotKeys = [];
...
export function setQuestHookTextDebugState(params) {
    const slotMeta = params.slotMeta
        .slice(0, MAX_HOOK_DEBUG_SLOTS)
        .map((entry) => {
        const slotType = entry.slotType === "worldPulse" ? "worldPulse" : "actionable";
        const source = entry.source === "llm" ? "llm" : "default";
        return {
...
export function runQuestEconomyTick(input) {
    ...
    if (focusQuest) {
        ...
        if (delta > 0) {
            ...
            focusQuest.urgency = clampInt(focusQuest.urgency + (input.classification === "reckless" ? 6 : -2) + Math.round(input.temporalSignal.residualTraceHeat / 40), 0, 100);
    ...
    for (const pressure of orderedPressures) {
        ...
        const score = pressure.intensity +
        ...
        state = {
            ...state,
            nextQuestSeq: state.nextQuestSeq + 1,
            worldPressures: state.worldPressures.map((entry) => entry.pressureId === pressure.pressureId
                ? {
                    ...entry,
