...
import { assertAgentAllowed, parseTrpgRuntimeConfig, resolveWorldRootForContext, } from "../../config.js";
...
import { buildCheckpoint1Panel, collectPanelRouteActionIds, parsePanelCustomId, } from "../../runtime-core/panel-mvp.js";
...
import { buildFactionCanonFingerprint, buildFactionCanonReferenceIndexFromWorldSeed, detectFactionCanonScaffoldDrift, validateFactionCanon, } from "../../faction-canon.js";
import { createRuntimeCanonicalProvenance, driftStatusFromLoadStatus, } from "../../runtime-core/sync-meta.js";
...
import { ensureSessionPresentationState, ensureRuntimeMetadata, } from "../../runtime-core/types.js";
import { loadStructuredWorldFile } from "../../world-store.js";
import { SESSION_DATA_SECTIONS, consumeSessionResetConfirmation, copySectionData, deleteSectionDataFromWorkspace, ensureSessionWorkspace, issueSessionResetConfirmation, readSessionWorkspaceRecord, wipeSessionWorkspace, } from "../../runtime-core/session-workspaces.js";
...
function buildNewConfirmationActionHints(confirmToken) {
    return {
        yes: {
            label: "YES",
...
function buildSessionStartActionComponents(sessionId, actorId) {
    return {
        type: "actions",
        buttons: [
            {
...
function buildSessionResumeActionComponents(sessionId, actorId) {
    return {
        type: "actions",
        buttons: [
            {
...
function sanitizeLegacyBootstrapTemplateText(value) {
    ...
    const hasForbidden = /\bpart\s*a\b|\bpart\s*b\b/i.test(normalized) ||
...
function clampTraceTailCount(value, fallback) {
...
function summarizeTraceData(value) {
...
function traceTailPayload(session, tailCount, includeData) {
...
function toSeedDiagnostics(issues, sourcePath) {
...
async function loadRuntimeCanonicalProvenance(params) {
    ...
    const seedStatus = params.seedBootstrap.status;
    const seed = params.seedBootstrap.validatedSeed;
    let canonStatus = "missing";
    let canonSourcePath = null;
    let canonFingerprint = null;
    let canonWorldId = null;
    let driftCounts = {
        addedInSeed: 0,
        missingInSeed: 0,
        changedScaffold: 0,
...
function resolveChannelKey(params, ctx) {
...
function resolveActorId(params, ctx) {
...
function resolveOwnerId(params, ctx) {
...
function resolveSessionContextId(params, ctx, channelKey) {
...
function resolveSectionList(value) {
    if (!Array.isArray(value) || value.length === 0) {
        return [...SESSION_DATA_SECTIONS];
    }
    const allowed = new Set(SESSION_DATA_SECTIONS);
    const selected = [];
...
function normalizeSession(session) {
    const nowIso = readString(session.updatedAt) || new Date().toISOString();
    const deterministicLoop = ensureDeterministicSceneLoopState(session.deterministicLoop, {
        sceneId: readString(session.sceneId) || "scene-bootstrap",
        nowIso,
    });
    const runtimeMetadata = ensureRuntimeMetadata(session.runtimeMetadata);
    const presentation = ensureSessionPresentationState(session.presentation);
    const sceneId = deterministicLoop.scene.sceneId;
    const ownerId = readString(session.ownerId) || "owner:unknown";
    const actionSeq = Math.max(0, readInteger(session.actionSeq) ?? 0, readInteger(session.turnIndex) ?? 0);
    const turnIndex = readInteger(session.turnIndex) ?? 0;
    const lastActionId = readString(session.lastActionId) || null;
    const lastActionSummary = sanitizeLegacyBootstrapTemplateText(readString(session.lastActionSummary)) || null;
    const normalized = {
        ...session,
        sceneId,
        ownerId,
        ...
        panelDispatch: {
            ...
            committedDispatchIds: Array.isArray(session.panelDispatch?.committedDispatchIds)
                ? session.panelDispatch.committedDispatchIds.filter((entry) => typeof entry === "string").slice(-32)
                : [],
        },
        panels: {
            fixed: {
...
function createRuntimeContext(worldRoot, cfg) {
...
async function syncMessageMetadata(params) {
    ...
    if (params.uiVersion !== undefined && params.uiVersion !== session.uiVersion) {
        throw new Error(`uiVersion mismatch while syncing metadata. expected=${String(session.uiVersion)} actual=${String(params.uiVersion)}`);
    }
...
function markDispatchExpired(session, nowIso) {
    ...
    return appendTraceEvent(next, createTraceEvent({
        lane: "adapter",
...
function isPendingDispatchExpired(session, nowIso) {
    const pending = session.panelDispatch.pending;
...
function hasCommittedDispatch(session, dispatchId) {
    return session.panelDispatch.committedDispatchIds.includes(dispatchId);
}
function preparePanelDispatch(params) {
    const mode = params.mode ?? (params.session.panels.main.messageId ? "edit" : "send");
    ...
    const availableButtons = collectPanelRouteActionIds(params.session).filter((actionId) => actionId !== "action.free_input.submit");
            ...
            actionable: {
                ...
                activeTop: questSummary.actionable.activeTop
                    ? {
                        ...
                        text: questSummary.actionable.activeTop.llmShortText ??
    ...
    const panelDispatchMessage = params.session.status === "active" ? `${panel.message} · 데이터 관리 명령: /trpg help` : panel.message;
    const payload = {
...
export function registerCheckpoint0LifecycleTools(api) {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Checkpoint 1 lifecycle entry for /trpg new. Creates owner-scoped session state and returns render payload for panel send.",
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Checkpoint 1 lifecycle entry for /trpg resume. Owner-only restore/recreate flow with uiVersion rotation and panel render payload.",
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Checkpoint 1 lifecycle entry for /trpg end. Owner-only session termination and panel finalization payload.",
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Owner-only verbose toggle for active session. Applies immediately and returns compact trace tail for Discord diagnostics.",
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Checkpoint 1 utility for /trpg save. Copies selected data sections from temp workspace into canonical world root.",
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Checkpoint 1 utility for /trpg load. Copies selected sections from canonical world root back into temp workspace.",
        parameters: SESSION_SECTION_TOOL_PARAMETERS,
        async execute(_toolCallId, params) {
            const gate = createGate({ cfg, ctx, api });
            if (!gate.ok) {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Checkpoint 1 utility for /trpg data-delete. Deletes selected sections from temp workspace without touching canonical files.",
        parameters: SESSION_SECTION_TOOL_PARAMETERS,
        async execute(_toolCallId, params) {
            const gate = createGate({ cfg, ctx, api });
            if (!gate.ok) {
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Owner-only panel interaction callback handler. Validates routing key from customId against state store, updates session, and returns edit payload.",
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                const nowIso = new Date().toISOString();
                session = appendTraceEvent(session, createTraceEvent({
                    lane: "adapter",
                ...
                const mode = nextSession.panels.main.messageId ? "edit" : "send";
                const prepared = preparePanelDispatch({
                    session: nextSession,
                    routes: resumed.routes,
                    mode,
    ...
    api.registerTool((ctx) => ({
        ...
        description: "Commit panel message metadata into state store after a message send/edit operation. Required for resume and update/edit loops.",
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                const committed = dispatchId
                    ...
                    : appendTraceEvent(normalizeSession(synced), createTraceEvent({
                        lane: "adapter",
                        type: "panel.commit.success",
                        tsIso: nowIso,
                        data: {
    ...
    api.logger.info("[trpg-runtime] checkpoint1 lifecycle tools registered: trpg_session_help, trpg_session_new, trpg_session_resume, trpg_session_end, trpg_session_verbose, trpg_session_save, trpg_session_load, trpg_session_data_delete, trpg_panel_interact, trpg_panel_message_commit");
}
