...
function toDiagnostics(diagnostics, sourcePath) {
...
function normalizeReachLevel(raw) {
...
function buildZoneLookup(params) {
    const out = {};
    const register = (raw) => {
...
function resolveZoneId(raw, zoneLookup) {
...
function classifyEventType(params) {
    ...
    if (/검문|통행|출입|checkpoint|access|gate|봉쇄|lockdown|inspection|permit/.test(op))
        return "access_change";
    if (/선적|하역|cargo|trade|market|supply|ledger|resource|shipment/.test(op))
        return params.major ? "trade_disruption" : "resource_shift";
    if (/순찰|배치|presence|patrol|escort|deployment/.test(op))
        return "presence_shift";
    if (/소문|rumor|whisper/.test(op))
        return "rumor_wave";
    if (/의식|봉인|징조|omen|weather|blight|ritual/.test(op))
        return "environmental_omen";
    if (/청문|심문|scrutiny|audit|tribunal|council|court/.test(op))
        return "institutional_scrutiny";
    if (/습격|폭력|violence|blood|raid|ambush/.test(op))
        return "violence_trace";
    if (params.visibility === "rumor")
        return "rumor_wave";
    if (params.major)
        return "trade_disruption";
    if (params.directness === "direct")
        return "presence_shift";
    return "resource_shift";
}
function visibilityFromPosture(posture) {
...
function reachFromPosture(posture) {
    if (posture === "assertive") {
        return "high";
    }
    if (posture === "low_profile") {
...
function instabilityFromFactionCanon(entry) {
...
function parseFactions(canon, zoneLookup) {
    const out = [];
    for (const entry of canon.factions) {
        ...
        const defaultZones = unique(entry.homeLocationIds
        ...
        const operationMethods = unique(entry.pressureAffinityIds.map((pressureId) => `stabilize pressure ${pressureId}`));
        const shortGoals = unique(entry.pressureAffinityIds.map((pressureId) => `maintain leverage on ${pressureId}`));
        const midGoals = unique(entry.homeLocationIds.map((locationId) => `hold influence around ${locationId}`));
        out.push({
            id: entry.factionId,
            name: entry.name,
            objectives: {
...
function normalizeAgenda(raw, faction) {
...
function triggerFromPrompt(prompt) {
    const low = (prompt ?? "").toLowerCase();
    if (low.includes("/downtime"))
        return "downtime";
    if (low.includes("/scene"))
        return "scene_transition";
    if (low.includes("/new") || low.includes("/reset"))
        return "session";
    return "turn";
}
export { triggerFromPrompt as inferFactionTriggerFromPrompt };
function policyFor(visibility, directness) {
    if (directness === "direct" && visibility !== "secret")
        return "drop_now";
    if (visibility === "rumor" || directness === "indirect")
        return "delay";
    return "silent";
}
function formatAccess(controls) {
...
function buildNoopTick(trigger) {
...
function buildCanonicalScaffoldProvenance(params) {
    const driftStatus = params.seedStatus === "invalid" || params.seedStatus === "error"
        ? "invalid_seed"
...
export function formatFactionPromptSummary(result) {
    const lines = [
    ...
    if (result.world_motion_summary.pressure.length) {
        lines.push("pressure:");
        for (const item of result.world_motion_summary.pressure.slice(0, 4))
            lines.push(`- ${item}`);
    }
    if (result.world_motion_summary.observations.length) {
        lines.push("observations:");
        for (const item of result.world_motion_summary.observations.slice(0, 4))
            lines.push(`- ${item}`);
    }
    if (result.world_motion_summary.npc_posture.length) {
        lines.push("npc_posture:");
        for (const item of result.world_motion_summary.npc_posture.slice(0, 4))
            lines.push(`- ${item}`);
    }
    if (result.world_motion_summary.rumors.length) {
        lines.push("rumors:");
        for (const item of result.world_motion_summary.rumors.slice(0, 4))
            lines.push(`- ${item}`);
    }
    if (result.world_motion_summary.access_changes.length) {
        lines.push("access_changes:");
        for (const item of result.world_motion_summary.access_changes.slice(0, 4))
            lines.push(`- ${item}`);
    }
...
export async function runFactionEngineTick(params) {
    const mode = params.input?.mode === "dry-run" ? "dry-run" : "read-only";
    ...
    const currentZoneId = resolveZoneId(s(travelState.current_zone), zoneLookup) ||
    ...
    const tick = trigger === "scene_transition"
    ...
    const pendingDrop = Array.isArray(eventsNode.pending_drop) ? [...eventsNode.pending_drop] : [];
    const delayed = Array.isArray(eventsNode.delayed_reflections) ? [...eventsNode.delayed_reflections] : [];
    const silent = Array.isArray(eventsNode.silent_state_changes) ? [...eventsNode.silent_state_changes] : [];
    const generated = [];
    if (advanced) {
        const ranked = factions
            .map((faction) => {
            const agenda = normalizeAgenda(toObj(agendaMap[faction.id]), faction);
            const score = agenda.pressure + Math.round(agenda.instability * 0.4) + Math.round(faction.heat * 0.25) - agenda.blockers.length * 3;
        ...
        for (let i = 0; i < ranked.length; i += 1) {
            const row = ranked[i];
            const major = row.score >= row.agenda.escalation_threshold.hard &&
            ...
            const directness = visibility === "secret"
                ? "silent"
            ...
            const event = {
                ...
                structural_consequences: [
                    ...
                    ...(major ? [{ kind: "access_control", key: affectedZones[0] || currentZoneId, delta: 1, summary: "access restrictions tighten" }] : []),
                    ...(row.faction.hooks.default_npcs.length
                        ? [{ kind: "npc_posture", key: row.faction.hooks.default_npcs[0], delta: major ? 2 : 1, summary: "npc posture hardens" }]
                        : []),
                ],
            ...
            row.agenda.pressure = clamp(row.agenda.pressure + (major ? 4 : 2) - row.agenda.decay_drift_rule.pressure_decay_per_tick, 0, 100);
            row.agenda.instability = clamp(row.agenda.instability + row.agenda.decay_drift_rule.instability_drift + (major ? 2 : 0), 0, 100);
            ...
            for (const c of event.structural_consequences) {
                if (c.kind === "zone_pressure" || c.kind === "district_tension") {
                    ...
                    const found = clocks.find((entry) => s(toObj(entry).id) === c.key);
                    if (found) {
            ...
            if (policy === "drop_now")
                pendingDrop.unshift(event);
            else if (policy === "delay")
                delayed.unshift(event);
            else
                silent.unshift(event);
            history.unshift(event);
        }
    }
    const focusZones = new Set(unique([currentZoneId, ...nearbyZoneIds]));
    const eventTouchesFocus = (event) => {
    ...
    const nextPressure = {
        ...
        access_controls: accessControls.slice(0, 20),
    };
    const nextAgendas = {
        meta: { schema_version: 1, last_updated: now },
        engine_state: {
