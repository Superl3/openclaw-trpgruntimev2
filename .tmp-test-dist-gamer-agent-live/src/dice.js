...
function resolveSeedMaterial(params) {
    const policy = params.input.seedPolicy === "fixed" ||
...
export function runDiceRoll(params) {
    ...
    const extraModifier = typeof params.input.modifier === "number" && Number.isFinite(params.input.modifier)
        ? Math.trunc(params.input.modifier)
        : 0;
    const repeatRaw = typeof params.input.repeat === "number" && Number.isFinite(params.input.repeat)
