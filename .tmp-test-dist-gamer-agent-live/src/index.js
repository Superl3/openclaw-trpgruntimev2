...
import { buildSceneComponents, COMPONENT_USAGE_GUIDE, } from "./discord-components.js";
...
function hasLegacyBootstrapTemplateLeak(value) {
    if (!value) {
        return false;
    }
    return (/PART\s*A|PART\s*B/i.test(value) ||
        /숨기고\s*있는\s*비밀/i.test(value) ||
        /(?:^|\n)\s*(?:1|2|3|4|5|6)\s*[\).:：\-]/.test(value));
}
function sanitizeLegacyBootstrapTemplateText(value) {
...
async function loadStatusPanelData(params) {
    ...
    const loadOrRepair = async (relativePath, fallbackRoot) => {
        try {
    ...
    if (statusNeedsRepair) {
        ...
        await fs.writeFile(resolveWorldAbsolutePath(params.worldRoot, "state/player-status.yaml"), renderStructuredContent(statusLoaded.format, statusRoot), "utf8");
    }
    ...
    if (inventoryNeedsRepair) {
        ...
        await fs.writeFile(resolveWorldAbsolutePath(params.worldRoot, "state/inventory.yaml"), renderStructuredContent(inventoryLoaded.format, inventoryRoot), "utf8");
    }
...
async function runCharacterBootstrapGate(params) {
    ...
    const introShown = sceneLoaded.exists
        ...
        : false;
    const runtimePhase = !characterCreated || !bootstrapComplete
        ? "BOOTSTRAP"
    ...
    if (readString(player.secret))
        contextLines.push(`Known 비밀: ${readString(player.secret)}`);
    if (readString(player.fear))
