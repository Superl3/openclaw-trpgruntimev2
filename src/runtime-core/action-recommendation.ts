...
import type { ActionFeasibility, DeterministicSceneLoopState } from "./scene-loop.js";
...
export type RecommendationFactorScore = {
  fit: number;
  weight: number;
  contribution: number;
  detail: string;
...
export type RecommendationActionScore = {
  actionId: string;
  baseScore: number;
...
function pressureLevel(loop: DeterministicSceneLoopState): number {
...
function goalLevel(loop: DeterministicSceneLoopState): { level: number; label: string } {
...
function traitFit(actionId: string, loop: DeterministicSceneLoopState): RecommendationFactorScore {
...
function goalFit(actionId: string, goalUrgency: number): RecommendationFactorScore {
...
function safetyFit(actionId: string, pressure: number): RecommendationFactorScore {
  const preferred: Record<string, number> = {
    "action.observe": 0.75,
    "action.move": 0.5,
    "action.wait": 0.65,
...
function pressureFit(actionId: string, pressure: number): RecommendationFactorScore {
  const preferred: Record<string, number> = {
    "action.observe": 0.45,
    "action.move": 0.7,
    "action.wait": 0.1,
...
function continuityFit(actionId: string, loop: DeterministicSceneLoopState): RecommendationFactorScore {
  let fit = 0.5;
...
function contextFit(
  ...
  paletteEntry: { availability?: ActionFeasibility | string } | null,
...
export function selectRuleBasedActionRecommendation(params: {
  loop: DeterministicSceneLoopState;
  palette: Array<{ actionId: string; availability?: ActionFeasibility | string }>;
  whimEnabled?: boolean;
  verboseMode?: boolean;
